...
/usr/bin/consul-cli service register --consul ${CONSUL} --id=${IPADDR}:${SERVICE_NAME} --tag=chaos-ready --tag=${AUTH64} --check="http:10s:http://${AUTH}@${IPADDR}:8080/health" ${SERVICE_NAME}
